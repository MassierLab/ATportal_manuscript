...
counts <- read.delim("Count_Subjects.txt")
ggplot(counts, aes(x=Module, y=Count, fill=Module, color=Gender))+ geom_col(position="dodge") + theme_bw() + scale_y_log10(expand=c(0,0)) + scale_x_discrete(limits=rev)+
  scale_fill_manual(values = c("#4f736a", "#e2c744","#ba5c28", "#f59b7c")) + coord_flip() 
